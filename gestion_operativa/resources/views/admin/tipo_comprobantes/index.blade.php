...
<div class="seperator-header layout-top-spacing">
    <div class="d-flex justify-content-between align-items-center">
        <h4 class="">Gestión de Tipos de Comprobante</h4>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#comprobanteModal" id="btnNuevoComprobante">
        ...
        <div class="statbox widget box box-shadow">
            <div class="widget-content widget-content-area">
                <div class="table-responsive full-height-row">
                    <table id="comprobantesTable" class="table table-striped table-bordered table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Nombre</th>
...
<!-- Modal para crear/editar comprobante -->
<div class="modal fade" id="comprobanteModal" tabindex="-1" aria-labelledby="comprobanteModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="comprobanteForm" action="" method="POST">
                @csrf
                <input type="hidden" id="comprobante_id" name="comprobante_id">
                <div class="modal-header">
                    <h5 class="modal-title" id="comprobanteModalLabel">Crear/Editar Tipo de Comprobante</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control" id="nombre" name="nombre" maxlength="20" placeholder="Ingrese el nombre (ej: Factura, Boleta)" required>
...
@section('scripts')
    ...
    function setupFormValidation() {
        ...
        // Validación nombre (requerido, máximo 20)
        $('#nombre').on('blur', function() {
            ...
            if (value.length === 0) {
                ...
                validateField($(this), 'El nombre no puede exceder 20 caracteres', function(value) {
    ...
    function validateForm() {
        ...
        if (nombre.length === 0) {
            ...
            if (!validateField($('#nombre'), 'El nombre no puede exceder 20 caracteres', function(value) {
                return value.length <= 20;
            })) {
                isValid = false;
                if (!firstErrorField) firstErrorField = $('#nombre');
    ...
    $(document).ready(function() {
        var table = $('#comprobantesTable').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: '{{ route('tipo-comprobantes.data') }}',
            columns: [
        ...
        $('#btnNuevoComprobante').click(function() {
            $('#comprobanteForm')[0].reset();
            $('#comprobante_id').val('');
            $('#comprobanteModalLabel').text('Nuevo Tipo de Comprobante');
            setupFormValidation();
        });
        ...
        $('#comprobanteForm').on('submit', function(e) {
            ...
            var url = id ? '/tipo-comprobantes/' + id : '/tipo-comprobantes';
    ...
    window.editTipoComprobante = function(id) {
        Swal.fire({
            title: 'Cargando...',
            allowOutsideClick: false,
        ...
        $.get('/tipo-comprobantes/' + id, function(data) {
            Swal.close();
            $('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
            $('.invalid-feedback').remove();
            ...
            $('#comprobanteModalLabel').text('Editar Tipo de Comprobante');
    ...
    window.deleteTipoComprobante = function(id) {
        Swal.fire({
            title: '¿Estás seguro?',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '/tipo-comprobantes/' + id,
                    type: 'DELETE',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(res) {
                        Swal.fire('Eliminado!', 'Tipo de Comprobante eliminado correctamente.', 'success');
                        $('#comprobantesTable').DataTable().ajax.reload();
                    },
                    error: function(xhr) {
                        var message = 'No se pudo eliminar el tipo de comprobante';
