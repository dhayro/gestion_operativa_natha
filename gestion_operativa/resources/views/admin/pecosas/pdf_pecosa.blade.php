...
<head>
    ...
    <title>PECOSA - {{ $pecosa->nro_documento }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="pecosa-container">
        ...
        <table class="header-table">
            <tr>
                ...
                <td class="title-cell">
                    ...
                    <div class="document-title">Parte de Entrega de Comisión de Obra</div>
                </td>
                <td class="correlativo-cell">
                    <div class="correlativo-box">
                        <div class="correlativo-label">DOCUMENTO N°</div>
                        <div class="correlativo-number">{{ $pecosa->nro_documento }}</div>
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table class="content-table">
            ...
            <tr>
                <td colspan="4" class="date-section">
                    FECHA: {{ $pecosa->fecha ? $pecosa->fecha->format('d/m/Y') : '-' }}
                </td>
            </tr>

            <!-- Cuadrilla y Empleado -->
            <tr>
            ...
            <tr>
                ...
                <td class="value">{{ $pecosa->cuadrillaEmpleado->cuadrilla->nombre ?? '-' }}</td>
                <td class="label">Empleado:</td>
                <td class="value">{{ $pecosa->cuadrillaEmpleado->empleado->nombre ?? '-' }}</td>
            </tr>
            <tr>
                <td class="label">Observaciones:</td>
                <td class="value" colspan="3">{{ $pecosa->observaciones ?? '-' }}</td>
            </tr>
        </table>
            ...
            <tbody>
                @foreach($pecosa->detalles as $detalle)
                    @php
                        ...
                        $material = $detalle->neaDetalle->material ?? null;
                        $unidad = '-';
                        
                        // Debug: obtener la unidad de medida
                        if ($material) {
                            // Intenta obtener la relación cargada
                            if ($material->relationLoaded('unidadMedida') && $material->unidadMedida) {
                                $unidad = $material->unidadMedida->abreviatura ?? '-';
                            } elseif ($material->unidad_medida_id) {
                                // Si no está cargada, buscarla
                                $um = \App\Models\UnidadMedida::find($material->unidad_medida_id);
                                $unidad = $um ? $um->abreviatura : '-';
                            }
                        }
                    @endphp
                    <tr>
                        <td><strong>{{ $material->codigo_material ?? '-' }}</strong><br>{{ $material->nombre ?? '-' }}</td>
        ...
        <table class="totales">
            <tr>
                ...
                <td class="value">S/. {{ number_format($pecosa->total_sin_igv, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td class="label">IGV (18%):</td>
                <td class="value">S/. {{ number_format($pecosa->igv_total, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td class="label total-value">TOTAL:</td>
                <td class="value total-value">S/. {{ number_format($pecosa->total_con_igv, 2, ',', '.') }}</td>
            </tr>
        </table>
        ...
        @if($pecosa->anulada)
            ...
            <table class="content-table">
                ...
                <tr>
                    ...
                    <td class="value" colspan="3">{{ $pecosa->motivo_anulacion ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="label">Anulada por:</td>
                    <td class="value">{{ $pecosa->usuarioAnulacion->name ?? 'Sistema' }}</td>
                    <td class="label">Fecha:</td>
                    <td class="value">{{ $pecosa->fecha_anulacion ? $pecosa->fecha_anulacion->format('d/m/Y H:i') : '-' }}</td>
                </tr>
            </table>
        @endif
        ...
        <table class="signature-table">
            <tr>
                <td>
                    ...
                    <div class="signature-title">Recibido por<br>Empleado</div>
                </td>
                <td>
                    <div class="signature-line"></div>
                    <div class="signature-title">Autorizado por<br>Supervisor</div>
                </td>
                <td>
                    <div class="signature-line"></div>
                    <div class="signature-title">Entregado por<br>Almacén</div>
                </td>
            </tr>
        </table>
        ...
        <table class="content-table">
            <tr>
                <td colspan="4" class="footer">
                    ...
                    <strong>Usuario:</strong> {{ $pecosa->usuarioCreacion->name ?? 'Sistema' }} | 
