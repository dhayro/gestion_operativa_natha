...
class PapeletaController extends Controller
{
    ...
    public function getAsignacionesDisponibles(Request $request)
    {
        ...
        $papeletaEditId = $request->get('papeleta_id', null); // Para modo edición
        ...
        // PERO incluir si estamos editando esa papeleta específica
        $query->where(function ($q) use ($papeletaEditId) {
            $q->whereDoesntHave('papeletas', function ($subQ) {
            ...
            // O si estamos editando, permitir el vehículo actual
            if ($papeletaEditId) {
                $q->orWhereHas('papeletas', function ($subQ) use ($papeletaEditId) {
                    $subQ->where('id', $papeletaEditId);
                });
            }
        });
    ...
    /**
     * Debug: Obtener información sobre por qué el usuario no ve papeletas
     */
    public function debugPapeletas(Request $request)
    {
        $user = Auth::user();
        ...
        if ($empleado) {
            ...
            $info['empleado'] = null;
            $info['advertencia'] = 'No hay empleado asociado a este usuario';
        }
        
        // Total de papeletas en la BD
        $info['total_papeletas'] = Papeleta::count();
        
        // Papeletas que debería ver
        $papeletasQueShouldSee = Papeleta::paraUsuario($user->id)->get();
        $info['papeletas_visibles'] = count($papeletasQueShouldSee);
        
        // Papeletas donde es usuario_creacion_id
        $papeletasCreadas = Papeleta::where('usuario_creacion_id', $user->id)->get();
        $info['papeletas_creadas_por_usuario'] = count($papeletasCreadas);
        if (count($papeletasCreadas) > 0) {
            $info['detalle_papeletas_creadas'] = $papeletasCreadas->map(function($p) {
                return [
                    'id' => $p->id,
                    'correlativo' => $p->correlativo,
                    'miembros_json' => $p->miembros_cuadrilla,
        ...
        if ($empleado) {
            // Papeletas donde está en miembros_cuadrilla
            $papeletasEnMiembros = Papeleta::whereRaw(
                "JSON_CONTAINS(miembros_cuadrilla, ?)", 
                [json_encode((int)$empleado->id)]
            )->get();
            $info['papeletas_en_miembros_json'] = count($papeletasEnMiembros);
            
            // Papeletas donde es chofer
            $papeletasChofer = Papeleta::where('chofer_id', $empleado->id)->get();
            $info['papeletas_como_chofer'] = count($papeletasChofer);
            
            // Papeletas de su cuadrilla
            $papeletasCuadrilla = Papeleta::whereHas('asignacionVehiculo.cuadrilla.cuadrillaEmpleados', function($q) use ($empleado) {
                $q->where('empleado_id', $empleado->id)
                  ->where('estado', true);
            })->get();
            $info['papeletas_de_su_cuadrilla'] = count($papeletasCuadrilla);
        }
