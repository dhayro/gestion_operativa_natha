...
class CategoriaController extends Controller
{
    public function index()
    {
        return view('admin.categorias.index', [
            ...
            'breadcrumbs' => ['Configuración', 'Categorías'],
            'scrollspy' => 0,
            'simplePage' => 0
        ]);
    }
...
public function getData(Request $request)
{
            ...
            ->addColumn('action', function ($row) {
                return '<div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink' . $row->id . '">
                                <a class="dropdown-item" href="javascript:void(0);" onclick="editCategoria(' . $row->id . ')">Editar</a>
                                <a class="dropdown-item" href="javascript:void(0);" onclick="deleteCategoria(' . $row->id . ')">Eliminar</a>
                            </div>
                        </div>';
            })
    ...
    public function store(Request $request)
    {
        ...
        $categoria = Categoria::create($request->only('nombre', 'estado'));
    ...
    public function update(Request $request, $id)
    {
        ...
        $categoria = Categoria::findOrFail($id);
        $categoria->update($request->only('nombre', 'estado'));
        return response()->json(['success' => true, 'data' => $categoria]);
    }
    ...
    public function getCategoriasForSelect()
    {
        try {
