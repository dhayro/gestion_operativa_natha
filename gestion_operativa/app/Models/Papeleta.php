...
class Papeleta extends Model
{
    ...
    public function scopeParaUsuario($query, $userId)
    {
        ...
        // Admin y Supervisor ven TODAS las papeletas
        if ($user->hasRole('admin') || $user->hasRole('supervisor')) {
            return $query;
        }
        
        // Operarios y técnicos ven papeletas de su cuadrilla
        // Buscar el empleado del usuario por email
        $empleado = \App\Models\Empleado::where('email', $user->email)->first();
        
        if (!$empleado) {
            // Si no hay empleado asociado, solo ve sus propias papeletas creadas
            return $query->where('usuario_creacion_id', $user->id);
        }
        
        // El empleado ve todas las papeletas de cuadrillas donde es miembro
        return $query->whereHas('asignacionVehiculo.cuadrilla.cuadrillaEmpleados', function ($q) use ($empleado) {
