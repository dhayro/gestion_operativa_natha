...
<div class="seperator-header layout-top-spacing">
    <div class="d-flex justify-content-between align-items-center">
        <h4 class="">Gestión de Construcciones</h4>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#construccionModal" id="btnNuevoConstruccion">
        ...
        <div class="statbox widget box box-shadow">
            <div class="widget-content widget-content-area">
                <div class="table-responsive full-height-row">
                    <table id="construccionesTable" class="table table-striped table-bordered table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Nombre</th>
...
<!-- Modal para crear/editar construcción -->
<div class="modal fade" id="construccionModal" tabindex="-1" aria-labelledby="construccionModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="construccionForm" action="" method="POST">
                @csrf
                <input type="hidden" id="construccion_id" name="construccion_id">
                <div class="modal-header">
                    <h5 class="modal-title" id="construccionModalLabel">Crear/Editar Construcción</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control" id="nombre" name="nombre" maxlength="50" placeholder="Ingrese el nombre de la construcción" required>
                        </div>
                    </div>
                    <div class="row">
...
@section('scripts')
    ...
    $(document).ready(function() {
        var table = $('#construccionesTable').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            ajax: '{{ route('construcciones.data') }}',
            columns: [
        ...
        $('#btnNuevoConstruccion').click(function() {
            $('#construccionForm')[0].reset();
            $('#construccion_id').val('');
            $('#construccionModalLabel').text('Nueva Construcción');
            setupFormValidation();
        });

        $('#construccionForm').on('submit', function(e) {
            ...
            var id = $('#construccion_id').val();
            var url = id ? '/construcciones/' + id : '/construcciones';
        ...
        $('#construccionModal').on('hidden.bs.modal', function () {
            $('#construccionForm')[0].reset();
            $('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
            $('.invalid-feedback').remove();
        });
    ...
    window.editConstruccion = function(id) {
        Swal.fire({
            title: 'Cargando...',
            allowOutsideClick: false,
        ...
        $.get('/construcciones/' + id, function(data) {
            Swal.close();
            $('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
            $('.invalid-feedback').remove();
            $('#construccion_id').val(data.id);
            ...
            $('#construccionModalLabel').text('Editar Construcción');
            $('#construccionModal').modal('show');
            setupFormValidation();
        }).fail(function() {
            Swal.close();
    ...
    window.deleteConstruccion = function(id) {
        Swal.fire({
            title: '¿Estás seguro?',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        Swal.fire('Eliminado!', res.message || 'Construcción eliminada correctamente.', 'success');
                        $('#construccionesTable').DataTable().ajax.reload();
                    },
                    error: function(xhr) {
                        var message = 'No se pudo eliminar la construcción';
